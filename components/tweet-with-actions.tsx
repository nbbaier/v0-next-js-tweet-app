...
export function TweetWithActions({
  ...
  const handleToggleSeen = async () => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Failed to update seen status")
    } finally {
  ...
  return (
    <div className="flex flex-col items-center space-y-1 w-full">
      ...
      <div className="w-full max-w-[550px] flex justify-start mb-1">
        <span className="text-xs px-2 py-1 rounded-full bg-muted text-muted-foreground">Saved by: {submittedBy}</span>
      </div>
      ...
      <div className="w-full max-w-[550px] flex justify-end gap-2">
        <Button variant="outline" size="sm" onClick={handleToggleSeen} disabled={isTogglingSeenStatus}>
          {isTogglingSeenStatus ? "Updating..." : isSeen ? "Mark as Unseen" : "Mark as Seen"}
        </Button>
      ...
      {error && !dialogOpen && (
        <div className="w-full max-w-[550px]">
          <p className="p-2 text-xs text-red-600 bg-red-50 rounded dark:bg-red-900/20">{error}</p>
        </div>
      )}
